...
%%
%function [index_DA, index_mon_DA, index_mon_CT, index_CT, index_real_t, C_input, flag_DA, flag_CT] ...
%= MPC_hiearchical_approach_third_layer(T_DA,T_CT,T_real_t, days,noise)
% installing required external tools 
    ...
    N_real_t = T_real_t/p.Delta_CT; % 4;   % real time layer
    simStart = 1;
   ...
   load ('../data/8760h-01p/noise.mat');
    ...
    % out_CT = [PL_CT; PV_CT; WT_CT;HL_CT;v_CT; u_CT]; 
     ...
     % load the real data 
     [out_DA_real, out_CT_real] = load_data(T_DA, T_CT, 0);  % no noise

     PL_DA_real = out_DA_real(1,:);
     PV_DA_real = out_DA_real(2,:);
     WT_DA_real = out_DA_real(3,:);
     HL_DA_real = out_DA_real(4,:);
     v_DA_real = out_DA_real(5,:);
     u_DA_real = out_DA_real(6,:);
    % 
      
     PL_CT_real = out_CT_real(1,:);
     PV_CT_real = out_CT_real(2,:);
     WT_CT_real = out_CT_real(3,:);
     HL_CT_real = out_CT_real(4,:);
     v_CT_real = out_CT_real(5,:);
     u_CT_real = out_CT_real(6,:);
  ...
  %% MPC Closed loop simulations
    ...
    [var_real_t, state_real_t] = generate_vars(N_real_t);
...
for ii = simStart:simEnd
   ...
   if ii == rr 
    ...
    %% interpolate DA solution 
     ...
     %% Step II: Continuous trading start at 15 min before delivery
        ...
        sol_CT_prev(1) = Pc_DA_int(1);
        sol_CT_prev(2) = Pd_DA_int(1);
        sol_CT_prev(3) = Hc_DA_int(1);
        sol_CT_prev(4) = Hd_DA_int(1);
        ...
        sol_CT_prev = input_real_t;  % take previous real input instead of input CT 
        ...
        x_cls_CT = [x_cls_CT, [sol_CT{1}(2);sol_CT{2}(2)]];
        u_cls_CT = [u_cls_CT, input_CT];
        P_CT_var = [sol_CT{3}(2); sol_CT{4}(2); sol_CT{13}(2); sol_CT{8}(2)];
        H_CT_var = [sol_CT{5}(2); sol_CT{6}(2); sol_CT{12}(2)];
        vec = [sol_CT{13}(2); 0*sol_CT{8}(2); 0*sol_CT{12}(2)];
              Pgrid_Cls_CT = [Pgrid_Cls_CT, sol_CT{13}(2)];
        P_CHP_Cls_CT = [P_CHP_Cls_CT, sol_CT{8}(2)];
        H_GB_Cls_CT = [H_GB_Cls_CT, sol_CT{12}(2)];
       ...
       if ii>94  % discared first day 
        ...
        %% Step III: Real-time layer  
        real_time_profiles = [PL_CT_real(ii+1: ii+N_real_t); HL_CT_real(ii+1:ii+N_real_t); PV_CT_real(ii+1:ii+N_real_t); ...
        WT_CT_real(ii+1:ii+N_real_t); u_CT_real(ii+1:ii+N_real_t); v_CT_real(ii+1:ii+N_real_t)];

       

        % extrapolation for Nreal_t = 24h
        if N_real_t == N_pred_CT
             Pg_CT = sol_CT{13}(2:end);
             % take the second input in the sequence
             input_CT = [sol_CT{3}(2:N_real_t);sol_CT{4}(2:N_real_t); sol_CT{5}(2:N_real_t); sol_CT{6}(2:N_real_t)]; 

        end
        opt_real_t = setup_real_t(var_real_t,state_real_t, real_time_profiles, Pg_CT, input_CT', N_real_t, p);
        % re-Initialisation
        if ii == 1
        % initial condition of the mmes
        % take the second predicted state by CT as IC of the mmes to ensure feasibility
        SoC_init_real_t = sol_CT{1}(2);  
        H_init_real_t = sol_CT{2}(2);  
        x_measure = [SoC_init_real_t,H_init_real_t]; % at 12:00
        init_real_t = {SoC_init_real_t, H_init_real_t};
        end

        mpc_RT = setup_continuous_trading_fix_u1([],var_RT,state_RT,real_time_profiles, Pg_CT(kk:N_CT-1+kk),N_RT, p);
        % re-Initialisation
        init_CT = {SoC_init_CT, H_init_CT};
        [sol_CT,flag_CT] = mpc_CT1(init_CT);
        ...
        %input_real_t_prev = input_real_t; % store 15 min old input
        input_real_t = [sol_real_t{3}(1);sol_real_t{4}(1); sol_real_t{5}(1); sol_real_t{6}(1)]; 
        P_CHP = sol_real_t{8}(1);
        H_GB = sol_real_t{12}(1);
        P_real_t_var = [sol_real_t{3}; sol_real_t{4}; sol_real_t{13}; sol_real_t{8}];
        H_real_t_var = [sol_real_t{5}; sol_real_t{6}; sol_real_t{12}];
        %if ii>95  % discared the first day 
         [index1, C_input0, C_e] = total_index_real_t(input_real_t, input_CT,P_CHP, H_GB, sol_real_t{13}(1), Pg_CT, p);
         index_real_t = index_real_t + index1;
         C_input_RT = C_input_RT + C_input0;
         C_e_RT = C_e_RT + C_e;

        %end

        x_CT_prev = x_measure; % store 15 min old measurement

        %% simulate the closed-loop system
        [out1, out2] = dynamics(x_measure, input_real_t, p); 
       
        % update the measurements
      
        x_measure = [out1; out2];
        init_real_t = {x_measure(1), x_measure(2)};
        Pgrid_Cls_real_t = [Pgrid_Cls_real_t, sol_real_t{13}(1)];
        P_CHP_Cls_real_t = [P_CHP_Cls_real_t, sol_real_t{8}(1)];
        x = [x, x_measure];
        u = [u, input_real_t];
        
        % update DA measurements every 24h
        if ii == zz  
        x_DA_measure =  x_measure;
     ...
     %    plot(Hc_DA_int_Cls,'go-')
      ...
      %   plot(Hd_DA_int_Cls,'go-')
         ...
         subplot(2,5,7), cla
         hold on
         plot(Pgrid_Cls_CT,'bo-')
         hold on 
         plot(Pgrid_Cls_real_t, 'ro-')
         hold on 
         plot(Pg_DA_Cls,'go-')
         grid on
         ...
         % 
         subplot(2,5,8), cla
         plot(P_CHP_Cls_CT,'bo-')
         hold on
         plot(P_CHP_Cls_real_t, 'ro-')
 ...
 % debugging 
         % figure(4)
         % plot(uncertainties_CT(1,:))
         % hold on
         % plot(uncertainties_CT(2,:))
         % hold on
         % plot(uncertainties_CT(3,:))
         % hold on;
         % plot(uncertainties_CT(4,:))
         % grid on
         % legend('P_L','H_L', 'PV', 'WT')
% toc
