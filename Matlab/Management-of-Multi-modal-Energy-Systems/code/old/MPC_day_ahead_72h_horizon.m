...
%%
   ...
   %% Upper level MPC: Day ahead parameters
    ...
    N = 72;                             % discrete-time prediction horizon
    N24 = 24;
    %% Read measurement data
    % Load measurement data from (Monday) 2019-01-28
    load ('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\Pload_DA.mat');
    load ('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\Hload_DA.mat');
    ...
    P_WT = WT_DA(1:3*N24);     % in kW
    % Power measurements of PV generation 
    P_PV = PV_DA(1:3*N24);
    % Power demand 
    P_load = PL_DA(1:3*N24);    % in kW
    % Heat demand 
    H_load = HL_DA(1:3*N24);
    % Price measurements 
    v_data = v_DA(1:3*N24);                                  % purchasing price of electricity
...
sgtitle('72h simulation of gas and HSS')
