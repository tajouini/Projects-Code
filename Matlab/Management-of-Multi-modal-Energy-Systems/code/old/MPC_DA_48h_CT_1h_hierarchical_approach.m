...
%% Upper level MPC: Day ahead
 ...
 %% Setup figures
    ...
    set(findall(gcf,'-property','FontSize'),'FontSize',20)
    set(gcf, 'Position',  [200, -90, 1000, 800])
    sgtitle(f1,'48h-1h Day ahead simulations');
    sgtitle(f2,'48h-1h Closed-loop simulations');
...
% Continuous Trading (CT) Read measurement/forecast data
    ...
    load ('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\PL_CT_N4.mat');
    % Real load profile
    ...
    load('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\PV_CT_N4.mat');
    ...
    load('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\WT_CT_N4.mat');
    ...
    load('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\HL_CT_N4.mat');
    ...
    load('C:\Users\Jouini\Documents\Research\IFES-Multi-modale-Energy-systeme\multi-energy-mangement-mpc\data\8760h-01p\v_CT_N4.mat');
 ...
 %% load and generation profiles
    ...
    % figure(3)
    % 
    % sgtitle('Forecasted uncertainty in DA')
    % set(findall(gcf,'-property','FontSize'),'FontSize',20)
...
for ii = simStart:simEnd   % in every 1/4 hour
   ...
   if ii == rr % every 24h iterations, the upper level is triggered
    ...
    times_DA = ceil(ii/4):N_DA-1+ceil(ii/4);  % look 48h ahead
    ...
    monetary_index_DA(v_GAS_DA,v_DA_real(times24),u_DA_real(times24), ...
    [sol_DA{13}(1:24); sol_DA{8}(1:24); sol_DA{12}(1:24)], Delta_DA);
    ...
    for jj = 1:size(Pg_DA,2)
     ...
     %Pg_DA_rshp = [Pg_DA_rshp Pg_DA_rshp(1:4)];
