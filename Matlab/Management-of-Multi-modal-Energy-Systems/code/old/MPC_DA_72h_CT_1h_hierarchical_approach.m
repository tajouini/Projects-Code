...
% Continuous Trading (CT) Read measurement/forecast data
 ...
 %% load and generation profiles
    ...
    ax31 = subplot(4,1,1);
    plot(v_DA(1:sim_time))
    hold on
    plot(u_DA(1:sim_time))
    grid on
    title('Prices')
    ...
    ax32 = subplot(4,1,2);
    plot(PL_DA(1:sim_time));
    grid on
    hold on
    plot(HL_DA(1:sim_time));
    title('Load profile')
    xlabel('Step in (every 1h)')
    ylabel('Power in kW')
    xlim([1,sim_time])
...
for ii = simStart:simEnd   % in every 1/4 hour
   ...
   if ii == rr % every 24h iterations, the upper level is triggered
    ...
    monetary_index_DA(v_GAS_DA,v_DA_real(times24),u_DA_real(times24),[sol_DA{13}(1:24); sol_DA{8}(1:24); sol_DA{12}(1:24)], Delta_DA);

    % fixing the input trajectory to give it to the lower level
    %u_OL_rshp = [];
    Pg_DA_rshp = [];
    for jj = 1:size(u_OL,2)
       %u_OL_rshp = [u_OL_rshp repmat(u_OL(:,jj),1,Delta_DA*24/Delta_CT)];
       Pg_DA_rshp = [Pg_DA_rshp repmat(Pg_DA(jj),1,Delta_DA*1/Delta_CT)];
    end
       %u_OL_rshp = [u_OL_rshp u_OL(:,end)]; %zeros(m,Delta_DA*24/Delta_CT)];
       Pg_DA_rshp = [Pg_DA_rshp Pg_DA(:,end)*ones(1,Delta_DA*3/Delta_CT)];
