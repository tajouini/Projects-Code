...
%%
function [index_DA, index_CT, index_mon_DA, index_mon_CT] = MPC_hiearchical_approach_v2(T_DA,T_CT,days,noise, int_method)
...
for ii = simStart:simEnd
   ...
   if ii == rr 
    ...
    % take the measurement of the current time (12:00) of the MES
    ...
    next_day = 1:N24;
    x_OL(1,:) = sol_DA{1}(1:N24+1); % SOC
    x_OL(2,:) = sol_DA{2}(1:N24+1); % H
    % inputs
    ...
    index_DA = index_DA + setup_cost_DA(v_DA_real(next_day),u_DA_real(next_day),P_DA_var,H_DA_var, N24, p);
    index_mon_DA = index_mon_DA + ...
    monetary_index_DA(v_DA_real(next_day),u_DA_real(next_day),mon_var, p);
    ...
    if (int_method == "m2" && T_DA == 48)
        ...
        rr = rr + p.Delta_DA*N24/p.Delta_CT;  % trigger DA every 24h --> 24*4 (in min)
     ...
     %% Step II: Continuous trading
        ...
        [out1, out2] = dynamics(x_measure, input, p); % simulate the closed-loop system
        x_measure = [out1; out2];
