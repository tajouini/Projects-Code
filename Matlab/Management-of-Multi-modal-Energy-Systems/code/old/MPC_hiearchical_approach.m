...
%%
function [index_DA, index_CT, index_mon_DA, index_mon_CT] = MPC_hiearchical_approach(T_DA,T_CT,days,noise, int_method)
...
for ii = simStart:simEnd
   ...
   if ii == rr 
    ...
    x_OL_int(1,:) = sol_DA{1}(1:N_SC); % SOC
    x_OL_int(2,:) = sol_DA{2}(1:N_SC); % H
    ...
    u_OL_int(1,:) = sol_DA{3}(1:N_SC); % Pc
    u_OL_int(2,:) = sol_DA{4}(1:N_SC); % Pd
    u_OL_int(3,:) = sol_DA{5}(1:N_SC); % Hc
    u_OL_int(4,:) = sol_DA{6}(1:N_SC); % Hd
     ...
     Pg_DA_rshp = [];
            for jj = 1:size(Pg_DA,2)
                Pg_DA_rshp = [Pg_DA_rshp repmat(Pg_DA(jj),1,p.Delta_DA/p.Delta_CT)];
                 % interpolate_pts(x_OL(:,zz),u_OL(:,zz),delta,delta_low)
                 if (jj<=12 && ii==1)
                     x_DA_int = [];
                    x_DA_int = [x_DA_int  interpolate_pts(x_OL_int(:,jj),u_OL_int(:,jj),p)];
                 end
            end
    ...
    if (int_method == "m2" && T_DA == 48)
        ...
        % add the value of one previous hour for the first 15 min of CT
        Pg_DA_rshp = [sol_DA{13}(N_SC)  Pg_DA_rshp];
     ...
     %% Step II: Continuous trading start at 15 min before delivery
        ...
        if ii ==1 % 23:45 measurement
         % take 15 min old measurement
          x_CT_prev = x_DA_int(1:2,end-1);
          %x_CT_measure  = x_DA_int(1:2,end); % measurement at midnight
        end
        ...
        input_CT = [sol_CT{3}(2);sol_CT{4}(2); sol_CT{5}(2); sol_CT{6}(2)];  % take the second input in the sequence
        ...
        if ii ==1  % take the second predicted state by CT to ensure feasibility
        ...
        monetary_index_CT(v_CT_real(ii+1),u_CT_real(ii+1),vec, Pg_DA_rshp(kk+1), p);

        
        
         [out1, out2] = dynamics(x_CT_measure, input_CT, p); % simulate the closed-loop system
       
         [sol0,flag0] = mpc_CT({out1,out2}); % see if xmeasure satisfies the constraints

         [sol_CT{1}(2); sol_CT{2}(2)]==x_CT_measure;

        if  (flag0 >0) % last time the problem was feasible
            break;
            % index = ii;
            % sol_CT_feas = sol_CT;
            % input_CT_feas = [sol_CT_feas{3}(2);sol_CT_feas{4}(2); sol_CT_feas{5}(2); sol_CT_feas{6}(2)];
            % x_CT_feas = [sol_CT_feas{1}(2);sol_CT_feas{2}(2)];
            % x_CT_m = x_CT_measure; 
            % x_prev_feas = x_CT_prev;
        end 
        x_CT_prev = x_CT_measure; % take 15 min old measurement
        x_CT_measure = [out1; out2];
        x = [x, x_CT_measure];
        u = [u, input_CT];
        
        if ii == zz  % update DA measurements every 12h
