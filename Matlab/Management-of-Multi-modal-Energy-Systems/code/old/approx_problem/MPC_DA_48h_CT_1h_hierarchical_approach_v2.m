...
% Continuous Trading (CT) Read measurement/forecast data 
 ...
 %% load and generation profiles
    ...
    sgtitle('Uncertainty profile for CT')
    set(findall(gcf,'-property','FontSize'),'FontSize',20)
...
for ii = simStart:simEnd   % in every 1/4 hour
   ...
   if ii == rr % every 24h iterations, the upper level is triggered
    ...
    u_OL_rshp = [];
    Pg_DA_rshp = [];
    for jj = 1:size(u_OL,2)-1
       u_OL_rshp = [u_OL_rshp repmat(u_OL(:,jj),1,Delta_DA*24/Delta_CT)];
       Pg_DA_rshp = [Pg_DA_rshp repmat(Pg_DA(jj),1,Delta_DA*24/Delta_CT)];
    end
       u_OL_rshp = [u_OL_rshp u_OL(:,end) zeros(m,Delta_DA*24/Delta_CT)];
       Pg_DA_rshp = [Pg_DA_rshp Pg_DA(:,end) zeros(1,Delta_DA*24/Delta_CT)];
     ...
     %% step 2: Continuous trading
        ...
        mpc_CT = setup_continuous_trading_v2(var_CT,state_CT, uncertainties_CT, Pg_DA_rshp(kk:kk+N_CT-1), v_GAS_CT,N_CT, Delta_CT);
        % re-Initialisation 
