...
%% Network
    % Grid power (origina formulation)
    %Pgrid_new = sdpvar(1,N);
    ...
    constraints = [constraints, -eps1 <= beta_ESS(:) & beta_ESS(:) <= eps1];
    constraints = [constraints, -eps2 <= gamma_HSS(:) & gamma_HSS(:)<= eps2];
...
%% Set up HSS constraints
   ...
   %% Set up GB constraints
    ...
    constraints = [constraints, PgridMin <= Pgrid_new(:) <= PgridMax];
    

    %% Power balance 
    constraints = [constraints, Pgrid_new(:)+P_PV(:)+P_WT(:)+Pd(:)+P_CHP(:) == Pc(:)+P_EB(:)+P_load(:)];
    % Heat balance 
...
%% Day Ahead Cost function 
    ...
    cost_new = 0;
    for k=1:N
    C_e_new(k) = (v(k)*P_v(k)+u(k)*P_u(k))*Delta_t;
    C_ESS(k) =  Rho_ESS*(Pd(k)+Pc(k))*Delta_t;
    C_HSS(k) =  Rho_HSS*(Hd(k)+Hc(k))*Delta_t;
    C_GAS(k) = v_GAS*(P_CHP(k)/Eta_CHP+H_GB(k)/Eta_GB);
    C_bin_ESS(k) = lambda*(Pc(k)+Pd(k))^2*Delta_t;  % Christian implementation
    C_bin_HSS(k) = lambda*(Hc(k)+Hd(k))^2*Delta_t; % Christian implementation
    cost_new = cost_new + C_e_new(k) + C_ESS(k) + C_HSS(k) + C_GAS(k) ...
    + C_bin_ESS(k) + C_bin_HSS(k);
    end
   ...
   % ops = sdpsettings('solver','gurobi','verbose',5, 'NonConvex', 2);
    ...
    out = {SoC,Pc,Pd,P_CHP,P_EB, beta_ESS, H, Hc, Hd, gamma_HSS, H_EB, H_GB, P_v,P_u, cost_new};
    % Generate yalmip object for optimization in closed-loop
    mpc = optimizer(constraints,cost_new,ops,init,out);   
   ...
   % initial soc is 0.3 and initial storage is 600 (see paper)
    ...
    Hc_Sol = sol{8};
    Hd_Sol = sol{9};
    gamma_HSS_Sol = sol{10};
    H_EB_Sol = sol{11};
    H_GB_Sol = sol{12};
    Pgrid_Sol = sol{13} + sol{14};  % P_g = P_v + P_u
    cost_opt = sol{15};
    P_v_Sol = sol{13};
    P_u_Sol = sol{14};
