function constraints = setup_constraints_fix_u1(sol_prev,SOC_var, Heat_var, States, IC, N, p, Delta)
...
%% Set up HSS constraints
   ...
   %% Set up GB constraints
    ...
    % fix the first input
    %  sol_prev = {SoC,H,Pc,Pd,Hc,Hd,P_EB,P_CHP,I_ESS,I_HSS,H_EB,H_GB,Pgrid,H_CHP};
    constraints = [constraints, Pc(1) == sol_prev(1) ];
    constraints = [constraints, Pd(1) == sol_prev(2)];
    constraints = [constraints, Hc(1) == sol_prev(3)];
    constraints = [constraints, Hd(1) == sol_prev(4)];
