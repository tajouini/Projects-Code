...
load ('8760h-01p\DA_uncertainties.mat');
load ('8760h-01p\DA_real.mat');
% load noise sequences
load ('8760h-01p\noise');
% electricity prices
v_data_CT = readtable ('8760h-01p\Gro_handelspreise_201901010000_201912312359_Viertelstunde');
v_CT_real = transpose(str2double(v_data_CT{:,4}))*1e-2*1e-3;   % from Mwh to Kwh
% remove first 26 days
index = (26*N24+1+11)*4+1;
v_CT_real = v_CT_real(index:end); 
...
for i=1:1:length(PL_DA_real)-1
  d = (PL_DA_real(i+1)-PL_DA_real(i))/4;
...
PL_CT_real(end) = PL_DA_real(end);
HL_CT_real(end) = HL_DA_real(end);
PV_CT_real(end) = PV_DA_real(end);
WT_CT_real(end) = WT_DA_real(end);


% % start at midnight: after 12h
...
v_CT_real = 1.2*v_CT_real;  % 1.2*v_DA

u_CT_real = 0.8*0.9/1.2*v_CT_real;   % 0.8*u_DA = 0.8*0.9/1.2 v_CT
...
%% CT forcast data 
PL_CT = PL_CT_real + factor*PL_CT_real.*noise1_CT;  % 10 procent

PL_CT_N4 = gen_grow_noise_CT(N4, noise1_CT, PL_CT_real);
PL_CT_N96 = gen_grow_noise_CT(N96, noise1_CT, PL_CT_real);
PL_CT_N48 = gen_grow_noise_CT(N48, noise1_CT, PL_CT_real);
%PL_CT_N192 = gen_grow_noise_CT(N192, noise1_CT, PL_CT_real);


HL_CT = HL_CT_real + factor*HL_CT_real.*noise2_CT;  % 10 procent
HL_CT_N4 = gen_grow_noise_CT(N4, noise2_CT, HL_CT_real);
HL_CT_N96 = gen_grow_noise_CT(N96, noise2_CT, HL_CT_real);
HL_CT_N48 = gen_grow_noise_CT(N48, noise2_CT, HL_CT_real);
%HL_CT_N192 = gen_grow_noise_CT(N192, noise2_CT, HL_CT_real);

v_CT = v_CT_real + factor*v_CT_real.*noise3_CT;     % 10 percent
v_CT_N4 = gen_grow_noise_CT(N4, noise3_CT, v_CT_real);
v_CT_N48 = gen_grow_noise_CT(N48, noise3_CT, v_CT_real);
v_CT_N96 = gen_grow_noise_CT(N96, noise3_CT, v_CT_real);
%v_CT_N192 = gen_grow_noise_CT(N192, noise3_CT, v_CT_real);
...
PV_CT = PV_CT_real + factor*PV_CT_real.*noise4_CT;  % 10 procent
PV_CT_N4 = gen_grow_noise_CT(N4, noise4_CT, PV_CT_real);
PV_CT_N48 = gen_grow_noise_CT(N48, noise4_CT, PV_CT_real);
PV_CT_N96 = gen_grow_noise_CT(N96, noise4_CT, PV_CT_real);
%PV_CT_N192 = gen_grow_noise_CT(N192, noise4_CT, PV_CT_real);

WT_CT = WT_CT_real + factor*WT_CT_real.*noise5_CT;  % 10 procent
WT_CT_N4 = gen_grow_noise_CT(N4, noise5_CT, WT_CT_real);
WT_CT_N48 = gen_grow_noise_CT(N48, noise5_CT, WT_CT_real);
WT_CT_N96 = gen_grow_noise_CT(N96, noise5_CT, WT_CT_real);
%WT_CT_N192 = gen_grow_noise_CT(N192, noise5_CT, WT_CT_real);
...
save("CT_uncertainties.mat",...
    ...
    "WT_CT", "WT_CT_N4", "WT_CT_N48", "WT_CT_N96",  "u_CT", "u_CT_N4","u_CT_N48", "u_CT_N96"); 

save("CT_real.mat","PL_CT_real", "HL_CT_real","v_CT_real","u_CT_real", "PV_CT_real", "WT_CT_real")
...
function  X = gen_grow_noise_CT(N, noise, real)
    relGrowth = linspace(0,0.2*N/(24*4),N); % from 0% to 20% noise at-- 24h
